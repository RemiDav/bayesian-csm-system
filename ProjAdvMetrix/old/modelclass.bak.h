...
void compute_pi(cublasHandle_t handle,Theta * dParticles){
	...
	//trsmbatched():  X = a B inv(A) if SIDE_RIGHT;   with a=-1 ; B = B;  A = Gamma;  The solution overwrites B.
	double alpha;
	alpha = -1.0;
...
/*--------Compute Omega = inv(Gamma)' . Sigma . inv(Gamma) ------------*/
void compute_omega(cublasHandle_t handle,Theta * dParticles){
	cublasStatus_t stat;
	printf("Compute Omega = inv(Gamma)' . Sigma . inv(Gamma) .\n");
	//generate array of adresses
	arrayTheta arrTheta;
	for(int i=0; i<(NUM_N*NUM_J);i++){
		arrTheta.gamma[i]=(double *) dParticles[i].gamma;
		arrTheta.omega[i]=(double *) dParticles[i].omega;
	}
	arrTheta.copyToDevice();
	//Copy sigma values to omega values (on the device)
	copy_sigma_to_omega<<<NUM_J,NUM_N>>>(dParticles);


	//Compute first Sigma . inv(Gamma)
	//trsmbatched():  X = a B inv(A) if SIDE_RIGHT;   with a=1 ; B = Sigma;  A = Gamma;  The solution overwrites B.
	double alpha;
	alpha = 1.0;
	stat = cublasDtrsmBatched(handle, CUBLAS_SIDE_RIGHT, //Gamma is on the right
						CUBLAS_FILL_MODE_UPPER, //Gamma is upper triangular
                        CUBLAS_OP_N, //Gamma does not need transposition
                        CUBLAS_DIAG_UNIT,//Gamma's diag is filled with 1
                        NUM_ENDOG,NUM_ENDOG,//Sigma is NUM_EXOG x NUM_ENDOG (K x M in Greene 10.6)
						&alpha, //the result is multiplied by -1
						(double **)arrTheta.dGamma, NUM_ENDOG, //Array of A matrices
                        (double **)arrTheta.dOmega, NUM_ENDOG, //Array of B matrices (and output)
                        NUM_J*NUM_N //Number of particles
						);
	if (stat != CUBLAS_STATUS_SUCCESS) {
        printf ("cublasDtrsmBatched failure\n");
	...
	//Compute then: inv(Gamma)' . [Sigma . inv(Gamma)]
	//trsmbatched():  X = a inv(A)'.B  if SIDE_LEFT & OP=Transpose;   with a=1 ; B = [Sigma . inv(Gamma)];  A = inv(Gamma);  The solution overwrites B.
	stat = cublasDtrsmBatched(handle, CUBLAS_SIDE_LEFT, //Gamma is on the right
						CUBLAS_FILL_MODE_UPPER, //Gamma is upper triangular
                        CUBLAS_OP_T, //Gamma does not need transposition
                        CUBLAS_DIAG_UNIT,//Gamma's diag is filled with 1
                        NUM_ENDOG,NUM_ENDOG,//Sigma is NUM_EXOG x NUM_ENDOG (K x M in Greene 10.6)
						&alpha, //the result is multiplied by -1
						(double **)arrTheta.dGamma, NUM_ENDOG, //Array of A matrices
...
public:
	...
	__host__ void DrawTheta(){
			...
			for(int n=0;n<num_n;n++){
				...
				for(int b_i=0; b_i<tempTheta.num_exog; b_i++){
					for(int b_j=0; b_j<tempTheta.num_endog; b_j++){
						tempTheta.beta[b_j*NUM_EXOG+b_i] = gsl_ran_gaussian(r,VAR_B);
					}
				}
				/*Gamma*/
				for(int b_i=0; b_i<tempTheta.num_endog; b_i++){
					for(int b_j=0; b_j<tempTheta.num_endog; b_j++){
						if(b_i==b_j){
							...
							tempTheta.gamma[b_j*NUM_ENDOG+b_i] = gsl_ran_gaussian(r,VAR_G);
						}
					}
				}
				/*Sigma*/
				//To generate sigma, I use a Wishart distribution Sigma = X'.X
				for(int b_i=0; b_i<tempTheta.num_endog; b_i++){
					for(int b_j=0; b_j<NUM_ENDOG; b_j++){
						tempTheta.Xsigma[b_j*NUM_ENDOG+b_i]= gsl_ran_gaussian(r,1);
					}
				}
				...
				for(int b_i=0; b_i<NUM_ENDOG;b_i++){
					det_inv_omega = tempTheta.gamma[b_i*NUM_ENDOG+b_i] * tempTheta.invLsigma[b_i*NUM_ENDOG+b_i] * tempTheta.invLsigma[b_i*NUM_ENDOG+b_i] * tempTheta.gamma[b_i*NUM_ENDOG+b_i] ;
				}
				...
				//Compute Sigma = alpha . X'X  -------------- to be deleted
				...
				double alpha = 1;
				double beta = 0;
				stat = cublasDgemm(handle,CUBLAS_OP_T,CUBLAS_OP_N,NUM_ENDOG,NUM_ENDOG,NUM_ENDOG,&alpha,
					(double *) &(dparticlesLm1[(j * num_n)+n].Xsigma),NUM_ENDOG,
					(double *) &(dparticlesLm1[(j * num_n)+n].Xsigma),NUM_ENDOG,
					&beta,
					(double *) &(dparticlesLm1[(j * num_n)+n].sigma),NUM_ENDOG //saves result in dparticlesLm1[(j * num_n)+n].sigma
				);
		...
		for(int i=0; i<2; i++){
			...
			for(int b_i=0; b_i<hparticlesLm1[0].num_endog; b_i++){
				for(int b_j=0; b_j<hparticlesLm1[0].num_endog; b_j++){
					printf("%.3f ",hparticlesLm1[i].sigma[IDX2C(b_i,b_j,NUM_ENDOG)]);
				}
			printf("\t");
			}
			printf("\n");
		...
		compute_omega(handle,dparticlesLm1);
		/*--------Compute Omega ------------*/
		...
		//Print Omega for 2 first particles
		for(int i=0; i<2; i++){
			printf("[%d] Omega (%d x %d):\n",i,hparticlesLm1[0].num_endog,hparticlesLm1[0].num_endog);
			for(int b_i=0; b_i<hparticlesLm1[0].num_endog; b_i++){
				for(int b_j=0; b_j<hparticlesLm1[0].num_endog; b_j++){
					printf("%.3f\t ",hparticlesLm1[i].omega[IDX2C(b_i,b_j,NUM_ENDOG)]);
				}
			printf("\n");
			}
			printf("\n");
	...
	__host__ void computeVariance(){
		...
		for(int i = 0; i < (NUM_J * NUM_N); i++) {
			...
			for(int j = 0; j< (NUM_ENDOG*NUM_ENDOG);j++){
				...
				sum.omega[j]+= hparticlesLm1[i].omega[j];
				sumsq.omega[j]+= hparticlesLm1[i].omega[j] * hparticlesLm1[i].omega[j];
			}
			/*invLSigma*/
		...
		for(int i = 0; i< (NUM_ENDOG*NUM_ENDOG);i++){
			...
			VarianceTheta.omega[i]= sumsq.omega[i] / (num_j * num_n) - (sum.omega[i]*sum.omega[i]) / (num_j *num_j * num_n * num_n) ;
	...
	__host__ void computeMean(){
		...
		for(int i = 0; i< (NUM_ENDOG*NUM_ENDOG);i++){
			...
			mean.omega[i]= sum.omega[i] / (num_j * num_n);
		}
		printf("\nMean Beta:\n%.4f\t%.4f\n%.4f\t%.4f\n%.4f\t%.4f\n%.4f\t%.4f\n",
		...
		printf("\nMean Omega:\n%.4f\t%.4f\n%.4f\t%.4f\n",
			mean.omega[0],mean.omega[2],
			mean.omega[1],mean.omega[3]	
		);
	...
	__host__ void mutate(){
		...
		for(int i = 0; i < (NUM_J * NUM_N); i++) {
			...
			for(int b_i=0; b_i<NUM_ENDOG; b_i++){
				for(int b_j=0; b_j<NUM_ENDOG; b_j++){
					if(b_i<b_j){
						hparticlesL[i].gamma[b_j*NUM_ENDOG+b_i] = hparticlesLm1[i].gamma[b_j*NUM_ENDOG+b_i] + gsl_ran_gaussian(r,sqrt(VarianceTheta.gamma[b_j*NUM_ENDOG+b_i])/red_coef);
					}
				}
			}
			//Step in omega space
			for(int b_i=0; b_i<NUM_ENDOG; b_i++){
				for(int b_j=0; b_j<NUM_ENDOG; b_j++){
					if(b_i==b_j){
						hparticlesL[i].omega[b_j*NUM_ENDOG+b_i] = hparticlesLm1[i].omega[b_j*NUM_ENDOG+b_i] * sqrt(gsl_ran_chisq(r,1));
					}
				}
			}
		...
		kHastingsRatio<<<NUM_J,NUM_N>>>(devStates,s,dparticlesLm1,dparticlesL,dYendog.dData,dXexog.dData);
		cudaDeviceSynchronize();
